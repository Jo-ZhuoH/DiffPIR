...
model_name              = 'model002000'  # model000000, ema_0.9999_000400, model000400
# diffusion_ffhq_10m, 256x256_diffusion_uncond; set diffusino model

testset_name            = 'train_2'    # set testing set, 'demo_li_lower', 'imagenet_val' | 'ffhq_val' | 'demo_test
# true_y_name             = 'demo_li_upper'    # set ground truth set
...
sigma                   = max(0.001,noise_level_img)  # noise level associated with condition y todo: check
...
n_channels              = 3                 # fixed: 3
cwd                     = ''
model_zoo               = os.path.join(cwd, 'model_zoo')    # fixed
testsets                = os.path.join(cwd, 'testsets')     # fixed
results                 = os.path.join(cwd, 'results')      # fixed
result_name             = f'{testset_name}_{task_current}_{generate_mode}_{sr_mode}{str(test_sf)}_{model_name}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}'
...
# H_path = os.path.join(testsets, true_y_name)  # H_path, for High-quality images
...
logger.info('model_name:{}, sr_mode:{}'.format(model_name, sr_mode))
...
for sf in test_sf:
    ...
    for k_index in range(k_num):
        ...
        def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
            logger.info(
                'eta:{:.3f}, zeta:{:.3f}, lambda:{:.3f}, inIter:{:.3f}, gamma:{:.3f}, guidance_scale:{:.2f}'.format(eta,
                                                                                                                    zeta,
                                                                                                                    lambda_,
                                                                                                                    inIter,
                                                                                                                    gamma,
            ...
            for idx, img in enumerate(L_paths):
                ...
                # img_H_path = img.replace("lower/down", "upper/up")
                # img_H = np.array(Image.open(img_H_path))
                img_H = util.imread_uint(img, n_channels=n_channels)
                img_H = util.modcrop(img_H, sf)  # modcrop

                img_L = np.array(Image.open(img))
                img_L_256 = np.array(Image.open(img))
                if sr_mode == 'blur':
                    if classical_degradation:
                ...
                img_L = img_L * 2 - 1
                img_L += np.random.normal(0, noise_level_img * 2, img_L.shape)  # add AWGN todo:no need noise_level_img
                img_L = img_L / 2 + 0.5
                ...
                # --------------------------------
                x = util.single2tensor4(img_L_256).to(device)
                ...
                for i in range(len(seq)):
                        ...
                        if seq[i] != seq[-1]:
                            if generate_mode == 'DiffPIR':
                                if sub_1_analytic:
                                    ...
                                    norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=x0, x_hat=x0,
                                                                                 measurement=2 * y - 1)
                                ...
                                down_sample = Resizer(x.shape, 1 / sf).to(device)
                                if generate_mode == 'DPS_y0':
                                    norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=x, x_hat=x0,
                                                                                 measurement=2 * y - 1)
                                    ...
                                    norm_grad, norm = utils_model.grad_and_value(operator=down_sample, x=xt, x_hat=xt,
                                                                                 measurement=y_t)
                    ...
                    if save_progressive and (seq[i] in progress_seq):
                        ...
                        if log_process:
                            logger.info(
                                '{:>4d}, steps: {:>4d}, np.max(x_show): {:.4f}, np.min(x_show): {:.4f}'.format(seq[i],
                                                                                                               t_i,
                                                                                                               np.max(
                                                                                                                   x_show),
                                                                                                               np.min(
                ...
                if calc_LPIPS:
                    ...
                    logger.info(
                        '{:->4d}--> {:>10s} -- sf:{:>1d} --k:{:>2d} PSNR: {:.4f}dB'.format(idx + 1, img_name + ext, sf,
                                                                                           k_index, psnr))
                ...
                if save_LEH:
                    ...
                    util.imsave(np.concatenate([img_I, img_E, img_H], axis=1),
                                os.path.join(E_path, img_name + '_x' + str(sf) + '_k' + str(k_index) + '_LEH' + ext))
